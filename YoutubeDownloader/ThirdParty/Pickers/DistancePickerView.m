...
- (void) addLabel:(NSString *)labeltext forComponent:(NSUInteger)component forLongestString:(NSString *)longestString {
    [labels setObject:labeltext forKey:[NSNumber numberWithInt:(int)component]];
    
    NSString *keyName = [NSString stringWithFormat:@"%@_%@", @"longestString", [NSNumber numberWithInt:(int)component]];
...
- (void)didMoveToWindow {
    ...
    for (int component=0; component<self.numberOfComponents; component++) {
        ...
        NSString *text = [labels objectForKey:@(component)];
        if (text) {
